 ...
 * @subpackage
 */
class PreProcessor_Optimizers_ArrayShortcut extends PreProcessor_Optimizer {
    ...
    public function parse(PreProcessor_File $file) {
        ...
        while ($it->valid()) {
            ...
            if (!is_array($token) && $token == '[') {
                if ((!is_array($last) && $last == ')') || (is_array($last) && $last[0] == T_VARIABLE)) {
                    //continue on
                } else {
                    $replace = array(array(T_ARRAY, 'array', 1), '(');
                    $start = $it->key();
                    $go = true;
                    $open = 1;
                    do {
